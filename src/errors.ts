...
export const TEAMSPEAK_SERVER_ERROR_IDS: Readonly<Record<string, WebSpeakErrorCode>> = {
  ...
  "524": "flooding",
  // Parameter (0x06xx). During the connect handshake the only size-checked value
  // the gateway sends is the nickname, so 1541 is reported as a nickname problem
  // instead of a generic parameter error.
...
export function normalizeTeamSpeakError(error: unknown): WebSpeakError {
  ...
  //    keyword heuristic below. Only 1541 used to be recognised, which pushed
  //    "banned", "server is full" and every other refusal into the generic
  //    fallback. The inline scan keeps the `id=515` shape working even when the
  //    SDK does not attach the id as a structured field.
  ...
  if (/invalid[\s_-]*parameter[\s_-]*size|parameter.{0,20}(size|out of range)|nickname.{0,20}(length|size)/.test(text)) {
...
export function clientConnectionFailureCode(error: WebSpeakError, serverPassword = ""): ClientConnectionFailureCode {
  ...
  const mapping: Record<WebSpeakErrorCode, ClientConnectionFailureCode> = {
    ...
    invalid_parameter_size: "INVALID_PARAMETER",
